...
//	unsigned char ParameterToPass;
	...
	xTaskCreate( vTaskLCD, (const signed portCHAR * const)"counter", configMINIMAL_STACK_SIZE,
			NULL, 3, NULL);

	xTaskCreate( vTaskOW, (const signed portCHAR * const)"ow", configMINIMAL_STACK_SIZE,
		NULL, 3, NULL);
